...
def main():
    ...
    for name, group in df_generated_tracks.groupby(['track_id', 'class']):
        ...
        # Shift tracks to center of image
        group['x_shift'] = group['x_norm'] - group['x_norm'].iloc[0]
        group['y_shift'] = group['y_norm'] - group['y_norm'].iloc[0]
        # Find min and max and abs of them
        min_data = min(group['x_shift'].min(), group['y_shift'].min())
        max_data = max(group['x_shift'].max(), group['y_shift'].max())
        max_data = max(abs(min_data), abs(max_data))
        # Scale to image size
        x = normalize_nparray(group['x_shift'].to_numpy(), 0, img_size, -max_data, max_data).astype(np.int32)
        y = normalize_nparray(group['y_shift'].to_numpy(), 0, img_size, -max_data, max_data).astype(np.int32)
